...
const SliderItem: React.FC<SliderItemProps> = ({
  ...
  }, [isDragging, isCompleted, points, gl, camera, raycaster, dragPlane, hitPoint, ndcPointer, setCanvasCursor]);

  useFrame((_, delta) => {
    if (!ballGroupRef.current) return;

    const targetOpacity = isCompleted ? 0 : 1;
    opacityRef.current = THREE.MathUtils.damp(
      opacityRef.current,
      targetOpacity,
      7,
    ...
    <group ref={rootGroupRef}>
      ...
      <group ref={ballGroupRef} position={startDotPos}>
        ...
        <Sphere
          args={[0.4, 16, 16]}
          onPointerOver={(e) => {
            if (isCompleted || opacityRef.current < 0.5) return;
            e.stopPropagation();
            setCanvasCursor(isDragging ? "grabbing" : "grab");
          }}
          onPointerOut={(e) => {
...
export const SmoothGlyphicSlider: React.FC<{
  ...
  isCompleted: boolean;
  indicatorPosRef: React.MutableRefObject<THREE.Vector3>;
}> = ({ setCanvasCursor, imageUrl, onProgress, onComplete, isCompleted, indicatorPosRef }) => {
  const samplesCount = 400;
  const points = useMemo(() => generatePathPoints(samplesCount), []);
...
export default function App() {
  ...
  // Play/pause the video when completion state changes
  useEffect(() => {
  ...
  const handleVideoEnded = () => {
    // Reset back to slider when the video finishes
    setIsVideoPlaying(false);
    setProgress(0);
  };

  return (
    <div className="relative w-screen h-screen bg-black touch-none select-none overflow-hidden">
      {/* 3D Scene Wrapper - fades out smoothly when video triggers */}
      <div
        className={`w-full h-full transition-opacity duration-700 ${isVideoPlaying ? "opacity-0 pointer-events-none" : "opacity-100"
          }`}
      >
        <Canvas camera={{ position: [0, 0, 7.5], fov: 48 }} style={{ cursor }}>
      ...
      {/* Screen-fitting Video Player */}
      <div
        className={`fixed inset-0 z-30 bg-black flex items-center justify-center transition-opacity duration-700 ${isVideoPlaying
